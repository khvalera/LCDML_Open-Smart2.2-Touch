...
Adafruit_GFX_Button btn_down, btn_up, btn_enter, btn_quit;

const int XP = 27, XM = 15, YP = 4, YM = 14;  //176x220 ID=0x6813
const int TS_LEFT = 500, TS_RT = -2600, TS_TOP = -2600, TS_BOT = 900;
...
bool touch_get_xy(void) {
  ...
  //bool pressed = (-p.z != 0 && -p.z != 0);
  if (pressed) {
    // Serial.printf ("y: %i, x: %i", p.y, p.x);
    // Serial.println("");
...
void lcdml_menu_control(void) {
  ...
  if (current_screen == 0) {
    ...
    btn_up.press(down && btn_up.contains(pixel_y, pixel_x));
    btn_enter.press(down && btn_enter.contains(pixel_y, pixel_x));
    btn_quit.press(down && btn_quit.contains(pixel_y, pixel_x));

    if (btn_down.justReleased())
      btn_down.drawButton();
    if (btn_down.justPressed()) {
      btn_down.drawButton(true);
      LCDML.BT_down();
    }
    if (btn_up.justReleased())
      btn_up.drawButton();
    if (btn_up.justPressed()) {
      btn_up.drawButton(true);
      LCDML.BT_up();
    }
    if (btn_enter.justReleased())
      btn_enter.drawButton();
    if (btn_enter.justPressed()) {
      btn_enter.drawButton(true);
      LCDML.BT_enter();
    }
    if (btn_quit.justReleased())
      btn_quit.drawButton();
    if (btn_quit.justPressed()) {
      btn_quit.drawButton(true);
      LCDML.BT_quit();
    }
  }
  if (current_screen > 0) {
